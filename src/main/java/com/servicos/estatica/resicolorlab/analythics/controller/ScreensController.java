...
public class ScreensController extends StackPane {
	...
	private static int tempoTransicao = 100;
		...
		if (screens.get(name) != null) { // screen loaded
			...
			if (!getChildren().isEmpty()) { // if there is more than one screen
				Timeline fade = new Timeline(new KeyFrame(Duration.ZERO, new KeyValue(opacity, 1.0)),
						new KeyFrame(new Duration(tempoTransicao), new EventHandler<ActionEvent>() {
							@Override
							public void handle(ActionEvent t) {
								...
								Timeline fadeIn = new Timeline(new KeyFrame(Duration.ZERO, new KeyValue(opacity, 0.0)),
										new KeyFrame(new Duration(tempoTransicao), new KeyValue(opacity, 1.0)));
								fadeIn.play();
							}
